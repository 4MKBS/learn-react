    ...
    render(){
      ...
      return(
        <div>
          ...
          <h1 className='h1'><span>Tik Tok - {children } {date.toLocaleTimeString(local)}</span></h1>
         {local === 'bn-BD' ?(
          <Button change={this.handleClick} local='ar-SA' show={false} enable={false} />
         ):(
          <Button change={this.handleClick} local='bn-BD' show={false} enable={false} />
         )}
        </div>
      );
    }
